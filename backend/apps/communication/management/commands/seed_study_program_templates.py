"""
Haftalık çalışma programı Meta + uygulama şablon taslaklarını oluşturur.

Örnekler:
  python manage.py seed_study_program_templates --list

  python manage.py seed_study_program_templates --kurum-id=1 --sube-id=1
...
from apps.communication.application.study_program_template_seed import (
    StudyProgramTemplateSeedService,
)


class Command(BaseCommand):
    help = 'Çalışma programı bildirimi için uygulama/Meta şablon taslaklarını oluşturur.'
    ...
    def handle(self, *args, **options):
        if options.get('list'):
            rows = StudyProgramTemplateSeedService.describe()
            for row in rows:
                self.stdout.write('')
            ...
            self.stdout.write('')
            self.stdout.write(self.style.SUCCESS(f'Toplam {len(rows)} çalışma programı taslağı.'))
            return
        ...
        result = StudyProgramTemplateSeedService.seed(
            kurum_id,
        ...
        self.stdout.write(self.style.SUCCESS('Çalışma programı şablon taslakları hazır.'))
