...
type Props = {
  ...
  /** false: yalnızca öğle arasını göster, diğer boşluklara "Ara" yazma */
  showGenericGaps?: boolean;
};
...
function gapLabel(
  ...
  return showGenericGaps ? 'Ara' : null;
}
          ...
          <div className={`gv-body${loading ? ' is-loading' : ''}`}>
            {requireSelection ? (
              ...
              <DayColumnsBoard
                ...
                showRoom={showRoom}
                colorBy={colorBy}
                showGenericGaps={showGenericGaps}
              />
            ) : (
                ...
                <table className="gv-grid">
                  ...
                  <tbody>
                    {slots.flatMap((slot, index) => {
                      const gap = index > 0 ? gapLabel(slots[index - 1].end, slot.start, showGenericGaps) : null;
                      const rows = [];
                      if (gap) {
                        rows.push(
            ...
            {dayCells.length ? (
              <div className="gv-dayboard-stack">
                {dayCells.flatMap((cell, index) => {
                  ...
                  const gap = previous
                    ? gapLabel(
                        previous.end || previousSlot?.end,
                        cell.start || slot?.start,
                        showGenericGaps,
                      )
                    : null;
                  return [
