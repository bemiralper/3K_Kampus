...
export default function TopBar({ settings, socialLinks }: TopBarProps) {
  ...
  const social = socialLinks.filter(l => ['instagram', 'facebook'].includes(l.platform) && l.aktif !== false);
  ...
  return (
    <div className="hidden h-10 items-center justify-between px-4 text-xs text-white md:flex lg:px-8" style={{ backgroundColor: LANDING_COLORS.navy }}>
      <div className="flex items-center gap-4">
        {phone && (
          <a href={`tel:${phoneDigits(phoneRaw)}`} className="flex items-center gap-1.5 hover:opacity-80">
            <PhoneIcon /> {phone}
          </a>
        )}
        {whatsapp && (
...
export function TopBarMobile({ settings }: { settings: SiteSettings | null }) {
  ...
  return (
    <div className="flex h-10 items-center justify-center gap-3 overflow-x-auto px-3 text-[11px] text-white sm:gap-6 sm:px-4 sm:text-xs md:hidden" style={{ backgroundColor: LANDING_COLORS.navy }}>
      {phone && (
        <a href={`tel:${phoneDigits(phoneRaw)}`} className="flex shrink-0 items-center gap-1.5 whitespace-nowrap">
          <PhoneIcon /> {phone}
        </a>
      )}
      {whatsapp && (
