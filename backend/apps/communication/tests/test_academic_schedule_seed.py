...
class AcademicScheduleSeedTest(TestCase):
    ...
    def test_seed_creates_app_meta_and_bindings(self):
        ...
        self.assertEqual(meta_veli.template_group, 'akademik:sinif_programi')
        teacher_meta = WhatsAppMetaTemplate.objects.get(
            channel_config=self.account, name='ogretmen_programi_personel',
        )
        self.assertEqual(teacher_meta.template_group, 'akademik:ogretmen_programi')
    ...
    def test_seed_backfills_stale_template_group(self):
        AcademicScheduleTemplateSeedService.seed(
            self.kurum.id,
            sube_id=self.sube.id,
            channel_config_id=self.account.id,
        ...
        meta.template_group = 'akademik'
        meta.save(update_fields=['template_group'])
        app = MessageTemplate.objects.get(kurum=self.kurum, name='Sınıf Programı — Veli')
        app.template_group = 'akademik'
        app.save(update_fields=['template_group'])
        AcademicScheduleTemplateSeedService.seed(
            self.kurum.id,
            sube_id=self.sube.id,
            channel_config_id=self.account.id,
        ...
        app.refresh_from_db()
        self.assertEqual(meta.template_group, 'akademik:sinif_programi')
        self.assertEqual(app.template_group, 'akademik:sinif_programi')
