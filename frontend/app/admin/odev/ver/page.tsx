...
import { StudentStep, AssignmentStep, ReviewStep, PrintPreview } from './components';
import type { Student, StudentResource, BookDetails, Content, Topic, Unit, SelectedContent, ContentTaskHistory } from './types';
import {
  fetchOgrenciList,
...
export default function OdevVerPage() {
  ...
  useEffect(() => {
    if (preselectedStudentId && students.length > 0 && !autoSelected) {
      const student = students.find(s => String(s.id) === preselectedStudentId);
      if (student) {
  ...
  }, [preselectedStudentId, students, autoSelected]);
  ...
  const fetchResources = async (sid: number) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setResources(result.data as StudentResource[]);
      }
