...
def occupied_day_columns(
    days: list[dict[str, Any]],
    group: dict[str, Any],
) -> list[tuple[dict[str, Any], list[dict[str, Any]]]]:
    """Dersi olmayan günleri dışa aktarma sütunlarından çıkarır."""
    columns = group_card_columns(days, group)
    return [(day, cards) for day, cards in zip(days, columns) if cards]
...
def export_schedule_csv(payload: dict[str, Any], *, filename: str) -> HttpResponse:
    ...
    for group in payload['groups']:
        ...
        if use_cards:
            ...
            for day, cards in occupied_day_columns(days, group):
...
def export_schedule_pdf(payload: dict[str, Any], *, filename: str, color_by: str = 'ders') -> HttpResponse:
    """WhatsApp eki ve indirme aynı HTML şablonunu kullanır."""
    from shared.export import style_manager as sm
    ...
    html_doc = build_schedule_pdf_html(payload, color_by=color_by)
    pdf_bytes = render_html_to_pdf(html_doc, landscape=True)
    response = HttpResponse(pdf_bytes, content_type='application/pdf')
    response['Content-Disposition'] = f'attachment; filename="{sm.safe_filename(filename)}.pdf"'
    return response
...
def export_schedule_xlsx(
    ...
    *,
    ...
    use_cards = export_uses_cards(payload)
    subject_kind = payload.get('subject_kind') or 'class'
    visible_day_count = max(
        (
            len(occupied_day_columns(days, group))
            for group in payload.get('groups') or []
        ),
        default=len(days) or 1,
    ) if use_cards else len(days)
    num_cols = max(2, (visible_day_count * 2) if use_cards else 1 + max(visible_day_count, 1))
    ...
    def write_class_grid(ws, group, start_row: int, *, show_title: bool = True) -> int:
        ...
        if use_cards:
            ...
            for day_i, (day, _cards) in enumerate(visible):
                rail_col = day_i * 2 + 1
                body_col = rail_col + 1
            ...
            depth = max((len(cards) for _day, cards in visible), default=0)

            for index in range(depth):
                for day_i, (_day, cards) in enumerate(visible):
                    ...
                    lesson_name = (card.get('lesson') or '').strip()
                    lesson = ws.cell(
                        row=r,
                        column=body_col,
                        value='\n'.join(part for part in (f'{index + 1}. Ders', lesson_name) if part),
                    )
            ...
            for day_i in range(len(visible)):
