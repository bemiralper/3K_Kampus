...
class AcademicScheduleTemplateDraft:
    event_key: str
    recipient_type: str
    app_name: str
    meta_name: str
...
class AcademicScheduleTemplateSeedService:
        ...
        for draft in drafts:
            ...
            if app_tpl:
                ...
                if stale:
                    ...
                    expected_group = template_group_for_event_key(draft.event_key)
                    if not dry_run and app_tpl.template_group != expected_group:
                        app_tpl.template_group = expected_group
                        app_tpl.save(update_fields=['template_group', 'updated_at'])
            ...
            if channel_config_id:
                ...
                if meta:
                    if (
                        ...
                        expected_group = template_group_for_event_key(draft.event_key)
                        if not dry_run and (meta.template_group or '') != expected_group:
                            MetaTemplateService.set_template_group(meta, expected_group)
    ...
    def describe(cls) -> list[dict[str, Any]]:
        rows = []
        for draft in list_academic_schedule_template_drafts():
