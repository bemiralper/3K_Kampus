"""
Yeni kayıt sözleşmesi (yönetici) Meta + uygulama şablon taslakları.

`ogrenci.kayit_sozlesme` olayı için PERSONEL metin şablonu üretir ve
Bildirim Şablonları eşlemesini kurar.
"""
...
KAYIT_SOZLESME_EVENT_KEY = 'ogrenci.kayit_sozlesme'

_APP_NAME_BY_RECIPIENT: dict[str, str] = {
    RecipientType.PERSONEL: 'Yeni kayıt sözleşmesi — Personel',
}


@dataclass(frozen=True)
class KayitSozlesmeTemplateDraft:
    event_key: str
    recipient_type: str
    app_name: str
    meta_name: str
...
def list_kayit_sozlesme_template_drafts() -> list[KayitSozlesmeTemplateDraft]:
    drafts: list[KayitSozlesmeTemplateDraft] = []
    for event in NOTIFICATION_EVENTS:
        if event.key != KAYIT_SOZLESME_EVENT_KEY:
            continue
        if event.module != MODULE_OGRENCI:
            continue
...
class KayitSozlesmeTemplateSeedService:
    """Kuruma kayıt sözleşmesi uygulama (+ Meta DRAFT) şablonlarını ekler."""
    ...
    def seed(
        ...
        *,
        ...
        for draft in drafts:
            ...
            if app_tpl:
                stale = (app_tpl.body or '') != draft.body_named
                if stale:
                    if dry_run:
        ...
        return {
            ...
            'bound': bound,
            'errors': errors,
            'event_keys': [KAYIT_SOZLESME_EVENT_KEY],
            'next_steps': [
                'ogrenci_kayit_sozlesme_personel taslağını Meta’ya gönderip onaylatın.',
                'Onay sonrası sözleşme aktif edilince pencere kapalıysa bu şablon gider.',
                'Alıcı yöneticileri Bildirim Şablonları’ndan işaretleyin.',
            ],
        }
    ...
    def describe(cls) -> list[dict[str, Any]]:
        rows = []
        for draft in list_kayit_sozlesme_template_drafts():
