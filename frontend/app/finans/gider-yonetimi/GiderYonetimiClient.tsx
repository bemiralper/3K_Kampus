...
interface GiderYonetimiClientProps {
  embedded?: boolean;
}

export default function GiderYonetimiClient({ embedded }: GiderYonetimiClientProps = {}) {
  const { homeHref, portalHomeHref } = useFinansPath();
  const { activeKurum, activeSube } = useKurum();
  const [tree, setTree] = useState<GiderKategorisiTreeItem[]>([]);
  ...
  const fetchTree = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res: GiderKategorisiTreeResponse = await giderKategoriService.tree(activeKurum.id, activeSube.id);
  ...
  const handleSeed = async () => {
    if (!activeKurum || !activeSube) return;
    try {
      const res = await giderKategoriService.seed(activeKurum.id, activeSube.id);
  ...
  const openEditAna = (item: GiderKategorisiTreeItem) => {
    setFormAd(item.ad);
  ...
  const openEditAlt = (alt: GiderAltKategori) => {
    setFormAd(alt.ad);
  ...
  const handleSave = async () => {
    ...
    try {
      if (drawer.type === "add-ana") {
        await giderKategoriService.create({ kurum_id: activeKurum.id, sube_id: activeSube.id, ad, ikon: formIkon, renk: formRenk, aciklama: formAciklama });
        showToast(`"${ad}" ana kategorisi oluşturuldu.`);
      } else if (drawer.type === "edit-ana" && drawer.itemId) {
        await giderKategoriService.update(drawer.itemId, { ad, ikon: formIkon, renk: formRenk, aciklama: formAciklama });
        showToast(`"${ad}" güncellendi.`);
      } else if (drawer.type === "add-alt" && drawer.parentId) {
        await giderKategoriService.create({ kurum_id: activeKurum.id, sube_id: activeSube.id, ad, parent_id: drawer.parentId, ikon: formIkon, renk: formRenk, aciklama: formAciklama });
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      await giderKategoriService.delete(drawer.itemId);
  ...
  const handleToggle = async (id: number) => {
    try {
      const res = await giderKategoriService.toggle(id);
      showToast(res.message);
      fetchTree();
    } catch (err: any) {
  ...
  if (!activeKurum) {
    return (
      <div className="empty-state" style={{ padding: "48px 20px" }}>
        ...
        <h4>Kurum Seçiniz</h4>
        <p>Gider kategorilerini görüntülemek için üst menüden bir kurum seçin.</p>
      </div>
    );
  }
  ...
  if (!activeSube) {
    return (
      <div className="empty-state" style={{ padding: "48px 20px" }}>
        <div className="empty-state-icon">🏫</div>
        <h4>Şube Seçiniz</h4>
        <p>Gider kategorileri şube bazlıdır. Üst menüden bir şube seçin.</p>
      </div>
    );
  }
        ...
        <div className="kat-content">
          {toplamAna === 0 && (
            <div className="empty-state">
              <div className="empty-state-icon">💸</div>
              <h4>Henüz Gider Kategorisi Yok</h4>
              <p>Gider kayıtlarınızı düzenlemek için önce kategorilerinizi oluşturun.</p>
