...
import { QUICK_ACCESS, scrollToSection, queueLandingScroll, hardNavigate } from '@/lib/landing-theme';
      ...
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 sm:gap-3 lg:grid-cols-7">
        {QUICK_ACCESS.map(card => (
          <button
            ...
            className="group flex flex-col items-center gap-1.5 rounded-xl border border-slate-200/80 bg-white p-3 shadow-sm transition hover:-translate-y-0.5 hover:border-[#0262a7]/30 hover:shadow-md sm:gap-2 sm:rounded-2xl sm:p-4"
          >
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#1e3a5f]/5 text-[#1e3a5f] transition group-hover:bg-[#0262a7] group-hover:text-white">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">{ICONS[card.icon]}</svg>
            </span>
