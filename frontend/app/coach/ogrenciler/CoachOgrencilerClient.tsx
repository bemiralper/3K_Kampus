...
const FILTER_OPTIONS: { id: FilterId; label: string; icon: string }[] = [
  ...
  { id: "needs_meeting", label: "Ödev takibi", icon: "📋" },
];

const VALID_FILTERS = new Set<FilterId>(FILTER_OPTIONS.map((f) => f.id));

function filterFromSearchParam(raw: string | null): FilterId {
  if (raw && VALID_FILTERS.has(raw as FilterId)) return raw as FilterId;
  return "all";
}
...
function StudentCard({
  ...
  const needsHomeworkFollowup = student.needs_meeting;
      ...
      <div className="coach-student-card-body">
        ...
        <div className="coach-student-signals">
          ...
          {needsHomeworkFollowup && (
            <span className="coach-signal is-warn">📋 Ödev takibi gecikti</span>
          )}
          {overdue > 0 && (
...
export default function CoachOgrencilerClient() {
  const { user } = useAuth();
  const userId = user?.id ?? 0;
  const router = useRouter();
  const searchParams = useSearchParams();
  const urlFilter = filterFromSearchParam(searchParams.get("filter"));
  ...
  const [activeFilter, setActiveFilter] = useState<FilterId>(urlFilter);
  ...
  const selectFilter = useCallback((next: FilterId) => {
    ...
    router.replace(qs ? `/coach/ogrenciler?${qs}` : "/coach/ogrenciler", { scroll: false });
  }, [router, searchParams]);
    ...
    <div className="coach-students-page">
      ...
      {userId > 0 && (
        <CoachReminderBanner
          ...
          onFilterNeedsMeeting={() => selectFilter("needs_meeting")}
          onFilterOverdue={() => selectFilter("overdue")}
        />
      )}
      ...
      {!loading && !error && students.length > 0 && (
        <section className="coach-students-stats" aria-label="Özet istatistikler">
          ...
          <button
            ...
            onClick={() => selectFilter("today_meeting")}
          >
