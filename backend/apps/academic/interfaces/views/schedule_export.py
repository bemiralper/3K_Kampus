...
def schedule_export_api(request):
    ...
    &export_format=csv|xlsx|json|pdf&layout=stacked|per_class_sheet
    ...
    if fmt == 'pdf':
        try:
            return export_schedule_pdf(payload, filename=filename, color_by=color_by)
        except Exception:
            return Response({'error': 'PDF oluşturulamadı.'}, status=500)

    return Response({'error': 'export_format csv, xlsx, json veya pdf olmalı.'}, status=400)
