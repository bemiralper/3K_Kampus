"""
Yeni kayıt sözleşmesi (yönetici) Meta + uygulama şablon taslaklarını oluşturur.

Örnekler:
  python manage.py seed_kayit_sozlesme_templates --list

  python manage.py seed_kayit_sozlesme_templates --kurum-id=1 --sube-id=1
...
class Command(BaseCommand):
    help = 'Kayıt sözleşmesi yöneticisi bildirimi için uygulama/Meta şablon taslaklarını oluşturur.'
    ...
    def handle(self, *args, **options):
        if options.get('list'):
            rows = KayitSozlesmeTemplateSeedService.describe()
            for row in rows:
                self.stdout.write('')
            ...
            self.stdout.write('')
            self.stdout.write(self.style.SUCCESS(f'Toplam {len(rows)} kayıt sözleşmesi taslağı.'))
            return
        ...
        self.stdout.write(self.style.SUCCESS('Kayıt sözleşmesi şablon taslakları hazır.'))
