"""
Özel ders yoklama Meta + uygulama şablon taslaklarını oluşturur.

Örnekler:
  python manage.py seed_ozel_ders_templates --list

  python manage.py seed_ozel_ders_templates --kurum-id=1 --sube-id=1 \\
...
class Command(BaseCommand):
    help = 'Özel ders bildirimi için uygulama/Meta şablon taslaklarını oluşturur.'
    ...
    def handle(self, *args, **options):
        if options.get('list'):
            rows = OzelDersTemplateSeedService.describe()
            for row in rows:
                self.stdout.write('')
            ...
            self.stdout.write('')
            self.stdout.write(self.style.SUCCESS(f'Toplam {len(rows)} özel ders taslağı.'))
            return
        ...
        self.stdout.write(self.style.SUCCESS('Özel ders şablon taslakları hazır.'))
