...
export interface OdevPaketleriPageContentProps {
  /** Ödev Ver sayfasının kökü — admin veya koç arayüzüne göre değişir. */
  verBasePath?: string;
}

export function OdevPaketleriPageContent({ verBasePath = "/admin/odev/ver" }: OdevPaketleriPageContentProps = {}) {
  ...
  const openEditModal = async (pkg: AssignmentPackage) => {
    // İçerikler (items) yalnızca detay endpoint'inde gelir; kaydederken pakette
    // var olan içerikleri yanlışlıkla silmemek için önce tam veriyi garantile.
    let full = pkg;
    if (!pkg.items) {
      try {
        const result = await fetchAssignmentPackage(pkg.id);
        if (result.success && result.data) {
          full = result.data;
          setPackages(prev => prev.map(p => p.id === full.id ? { ...p, ...full } : p));
        }
      } catch { /* silent — meta alanları için pkg fallback yeterli */ }
    }
    setSelectedPackage(full);
    setEditName(full.name);
    setEditDesc(full.description || "");
    setEditDers(full.ders_ad || "");
    setEditSinif(full.sinif_seviyesi || "");
    setShowEditModal(true);
  };

  const handleUpdateMeta = async () => {
    if (!selectedPackage || !editName.trim()) return;
    setSavingEdit(true);
    try {
      const result = await updateAssignmentPackage(
        selectedPackage.id,
        packagePayload(
          { name: editName.trim(), description: editDesc.trim(), ders_ad: editDers, sinif_seviyesi: editSinif },
      ...
      if (result.success && result.data) {
        ...
        flash("✅ Paket bilgileri güncellendi");
      } else {
                    ...
                    <div key={pkg.id} onClick={() => selectPackage(pkg)} style={{ background: isSelected ? "#fffbeb" : "white", border: isSelected ? "2px solid #f59e0b" : "1px solid #e2e8f0", borderRadius: 12, padding: "16px 20px", cursor: "pointer", transition: "all 0.15s" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                        ...
                        <div style={{ display: "flex", gap: 4, marginLeft: 8 }}>
                          <button onClick={(e) => { e.stopPropagation(); openEditModal(pkg); }} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 14, opacity: 0.5 }} title="Düzenle">✏️</button>
          ...
          {selectedPackage && (
            <div className="opk-detail" style={{ background: "white", borderRadius: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.08)", border: "1px solid #e2e8f0", overflow: "hidden" }}>
              ...
              <div style={{ padding: "16px 24px", borderBottom: "1px solid #f1f5f9", display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button onClick={() => openEditModal(selectedPackage)} style={{ padding: "8px 16px", background: "#f1f5f9", color: "#1e293b", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>✏️ Bilgileri Düzenle</button>
                ...
                <Link href={`${verBasePath}?package_id=${selectedPackage.id}`} style={{ padding: "8px 16px", background: "#10b981", color: "white", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6 }}>📤 Bu Paketten Ödev Ver</Link>
        ...
        <>
          ...
          <div className="opk-modal" style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", background: "white", borderRadius: 16, padding: 32, zIndex: 1001, boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Paket Adı *</label>
                <input type="text" value={editName} onChange={e => setEditName(e.target.value)} placeholder="Örn: Haftalık Matematik Paketi" style={{ width: "100%", padding: "10px 14px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 14, outline: "none" }} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Açıklama</label>
              ...
              <div className="opk-field-row" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Sınıf</label>
                  <select value={editSinif} onChange={e => setEditSinif(e.target.value)} style={{ width: "100%", padding: "10px 14px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 14, outline: "none", background: "white" }}>
                    <option value="">Seçiniz</option>
                    {sinifSeviyeleri.map(s => <option key={s.id} value={s.ad}>{s.ad}</option>)}
                  </select>
                </div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, marginTop: 24 }}>
              <button onClick={() => setShowEditModal(false)} style={{ padding: "10px 20px", background: "#f1f5f9", border: "none", borderRadius: 8, cursor: "pointer", fontSize: 14 }}>İptal</button>
              <button onClick={handleUpdateMeta} disabled={!editName.trim() || savingEdit} style={{ padding: "10px 24px", background: editName.trim() && !savingEdit ? "#f59e0b" : "#d1d5db", color: "white", border: "none", borderRadius: 8, cursor: editName.trim() && !savingEdit ? "pointer" : "default", fontSize: 14, fontWeight: 600 }}>{savingEdit ? "Kaydediliyor..." : "Kaydet"}</button>
            </div>
          </div>
        </>
      )}
