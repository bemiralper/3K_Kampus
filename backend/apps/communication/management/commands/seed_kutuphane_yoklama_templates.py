"""
Kütüphane yoklama Meta + uygulama şablon taslaklarını oluşturur.

Örnekler:
  python manage.py seed_kutuphane_yoklama_templates --list

  python manage.py seed_kutuphane_yoklama_templates --kurum-id=1 --sube-id=1 \\
...
class Command(BaseCommand):
    help = 'Kütüphane yoklama bildirimi için uygulama/Meta şablon taslaklarını oluşturur.'
    ...
    def handle(self, *args, **options):
        if options.get('list'):
            rows = KutuphaneYoklamaTemplateSeedService.describe()
            for row in rows:
                self.stdout.write('')
            ...
            self.stdout.write('')
            self.stdout.write(self.style.SUCCESS(f'Toplam {len(rows)} kütüphane yoklama taslağı.'))
            return
        ...
        self.stdout.write(
            f"meta DRAFT: +{len(result['created_meta'])} atlandı={len(result['skipped_meta'])} "
        ...
        self.stdout.write(self.style.SUCCESS('Kütüphane yoklama şablon taslakları hazır.'))
